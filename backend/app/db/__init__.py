...
class StubDatabase:
        ...
        self.broll_assets = [
            {
                ...
                "embedding": build_placeholder_vector(
                    "aerial drone shot coastal highway sunset", 512
                ),
            },
            {
                "id": "pixabay_992100",
                ...
                "embedding": build_placeholder_vector(
                    "business handshake in modern office", 512
                ),
            },
        ]
        self.knowledge_segments = [
            {
                ...
                "embedding": build_placeholder_vector(
                    "agents reasoning models keynote answer", 1536
                ),
            },
            {
                "id": "yt_seg_002",
                ...
                "embedding": build_placeholder_vector(
                    "cosine similarity semantic retrieval", 1536
                ),
            },
        ]
    ...
    def restore(self, snapshot: dict[str, Any]) -> None:
        self.api_keys = snapshot["api_keys"]
        self.usage_events = snapshot["usage_events"]
        self.usage_monthly = snapshot["usage_monthly"]
        self.query_logs = snapshot["query_logs"]
        ...
        *,
        ...
        if published_after is not None:
            rows = [
                row
                for row in rows
                if row["published_at"] >= published_after
            ]
        return deepcopy(rows[:limit])
...
async def get_db() -> AsyncIterator[StubDatabase]:
