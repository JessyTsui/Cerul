...
本文档列出 Cerul 项目当前所有待完成的任务，按依赖关系分组，用于规划多 worktree 并行开发。
...
| ID | 任务 | 依赖 | 预估改动范围 |
|----|------|------|-------------|
| T01 | Gemini Embedding 迁移 | 无 | `backend/app/embedding/`, `workers/`, `db/migrations/` |
| T02 | Better Auth 集成 | 无 | `frontend/lib/`, `frontend/app/login/`, `frontend/app/signup/`, `backend/app/auth/session.py` |
| T03 | 配置管理系统 | 无 | `config/`, `backend/app/config/` |
| T04 | Stub DB 清理 | 无 | `backend/app/db/stub.py`, `backend/app/auth/api_key.py` |
| T05 | Knowledge Pipeline 实现 | T01 | `workers/knowledge/` |
| T06 | YouTube 数据客户端 | 无 | `workers/common/sources/youtube.py` |
| T07 | 查询向量生成 | T01 | `backend/app/search/broll.py`, `backend/app/search/knowledge.py` |
| T08 | Knowledge 搜索答案生成 | T05, T07 | `backend/app/search/knowledge.py` |
| T09 | Scheduler 自动化 | T05 | `workers/scheduler.py` |
| T10 | Worker 重试机制 | T05 | `workers/worker.py` |
| T11 | B-roll 扩量至 10 万条 | T01, T07 | `scripts/seed_broll.py` |
| T12 | Pixabay 集成 | 无 | `workers/common/sources/pixabay.py`, `workers/broll/steps/` |
| T13 | Search Demo 页面 | T07 | `frontend/app/search/` |
| T14 | Pipelines Dashboard 页面 | T09 | `frontend/app/dashboard/pipelines/` |
| T15 | Stripe 端到端验证 | T02 | `backend/app/billing/`, `frontend/app/dashboard/settings/` |
| T16 | Docs 内容页面 | 无 | `frontend/app/docs/` |
| T17 | Codex/Claude Agent Skill | 无 | `skills/cerul-api/SKILL.md` |
| T18 | 速率限制增强 | 无 | `backend/app/middleware/rate_limit.py` |
